...
# and `pip install pyCBS` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "pyCBS", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n pyCBS -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "pyCBS", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "pyCBS", "YOUR_CONSOLE_SCRIPT"]
