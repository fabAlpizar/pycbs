...
def bakoules(Ec_X, Ec_Y, X=2, Y=3, beta=3.877):
    num = Ec_Y * ((X + 1)**-beta) - Ec_X * ((Y + 1)**-beta)
    den = ((X + 1)**-beta) - ((Y + 1)**-beta)
    return num / den


def halkier(Ec_X, Ec_Y, X=2, Y=3):
    num = Ec_X * X**3 - Ec_Y * Y**3
    den = X**3 - Y**3
    return num / den


def huh_lee(Ec_X, Ec_Y, X=2, Y=3, beta=0.220):
    num = Ec_Y * ((X + beta)**-3) - Ec_X * ((Y + beta)**-3)
    den = ((X + beta)**-3) - ((Y + beta)**-3)
    return num / den


def martin(Ec_X, Ec_Y, X=2, Y=3, beta=3.315):
    num = Ec_Y * ((X + 0.5)**-beta) - Ec_X * ((Y + 0.5)**-beta)
    den = ((X + 0.5)**-beta) - ((Y + 0.5)**-beta)
    return num / den


def oan(Ec_X, Ec_Y, beta=2.086):
    num = Ec_Y * 27 - (beta**3) * Ec_X
    den = 27 - (beta**3)
    return num / den


def truhlar(Ec_X, Ec_Y, X=2, Y=3, beta=2.751):
