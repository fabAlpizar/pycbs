...
def run_optimization(config_dict=None, output_file=None):
    """
    Run the optimizer. Returns a dict containing final parameters, final energy,
    history (list of cycles), and converged boolean.
    If output_file is provided the function will also write a compact optimization
    summary into that file via writer.write_optimization_summary(...).
    """
    # merge defaults
    cfg = dict(DEFAULTS)
    if config_dict:
    ...
    def parallel_optimize_parameters(current_params, cycle, displacement_factor, executor):
        ...
        results = []
        # progress per-parameter within cycle, leave=False to avoid clutter
        from tqdm import tqdm
        with tqdm(total=len(geo_init), desc=f"Cycle {cycle}", leave=False) as pbar:
            futures = [executor.submit(optimize_parameter, args) for args in args_list]
            for future in concurrent.futures.as_completed(futures):
                result = future.result()  # let exceptions bubble out
    ...
    try:
        for cycle in range(1, maxcycle + 1):
            print(f"\n>>> CYCLE {cycle}/{maxcycle}  (displacement factor = {displacement_factor:.6f})")
            ...
            for param_idx, param_values, energies in optimization_results:
                ...
                if len(energies) >= 3:
                    ...
                    if opt_energy < current_energy:
                        ...
                        print(f"  {param_name}: updated -> {opt_value:.6f}  E={opt_energy:.10f}")
                    else:
                        print(f"  {param_name}: no improvement")
                else:
                    print(f"  {param_name}: insufficient points for fit")
            ...
            if cycle > 1:
                ...
                print(f"  ΔE from last cycle: {energy_diff:.4e} Ha")
    ...
    # print final compact block
    print("\n" + "=" * 60)
    print("OPTIMIZER RESULTS (final)")
    print("=" * 60)
